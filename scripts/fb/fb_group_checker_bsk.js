...
let activeTool = 'ego-browser';

// Universal tool runner helper (supports ego-browser, bsk, playwright)
function runBsk(args) {
  const tool = (activeTool || 'ego-browser').toLowerCase();
...
function getActiveSession() {
  ...
  console.log(`[${activeTool}] No active session found. Starting a new session...`);
  const newSessionId = runBsk(['session', 'start']);
  console.log(`[${activeTool}] Started session: ${newSessionId}`);
  return newSessionId;
}
...
async function main() {
  ...
  console.log(`Configured Scraping Tool: ${activeTool}`);
