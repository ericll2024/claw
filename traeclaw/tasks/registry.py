...
TASK_WORKFLOWS: dict[str, tuple[WorkflowStep, ...]] = {
    ...
    "facebook.yesterday_summary": _steps(
        ("读取配置与工具", "从 fb_groups.json 读取监控群组列表及配置的抓取工具(ego-browser/bsk/playwright)。"),
        ("连接浏览器引擎", "通过配置的网页工具(ego-browser/bsk/playwright)连接并激活浏览器页面。"),
...
def list_tasks() -> list[TaskDefinition]:
    ...
    return [
        ...
        TaskDefinition(
            ...
            group="fb",
            description="支持配置网页抓取工具 (ego-browser/bsk/playwright) 自动增量监控 Facebook 群组发帖并生成摘要报告。",
            schedule_kind="daily",
            time_of_day="09:00",
            schedule_label="每天 09:00",
